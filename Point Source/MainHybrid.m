...
PSx=[0.5]; % Point Source x -Coordinate
PSy=[0.5]; % Point Source y- Coordinate
...
% Irregular polygon
%  xv = [1.0480 0.2180 0 0.7680 1.0480];
%  yv = [0.6993 1.1720 0.3582 0 0.2993]; 
%  Omega=[1 2 3 4 5];
...
costheta=-(((xi-PSx).*nx + (yi-PSy).*ny)./sqrt((xi-PSx).*(xi-PSx)+(yi-PSy).*(yi-PSy)))';
%Boundary condition for the reverberant solution V 
BC_P0
...
%%
stage=3
% CALCULATE THE BOUNDARY SOLUTION FOR HIGH FREQUENCIES EITHER VIA DEA/SHFA
MinDamp=imag(kfreq)*2;
if DEA==1
    DEAScriptPS
else
