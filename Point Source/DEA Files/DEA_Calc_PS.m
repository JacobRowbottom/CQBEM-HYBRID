function [VecInf,Tj,nSideEls,ThetaLocMat]=DEA_Calc_PS2(dx,waveno,xnodes,ynodes,Force,PSx,PSy,NQ,Omega,MinDamp)
...
% % %      
    ...
    Adamp(j)=imag(waveno)*2; %Adamp = 1; %imaginary part of k=wavenumber
    ...
    for iv=1:nVert(j)
        ...
        cSideEls(j,iv+1)=cSideEls(j,iv)+nSideEls(j,iv); %No. of elements on each edge %nEdge in Jacobs code 
...
%Helmholtz
    ...
    % waveno=(params.omega/params.speed)+1i*Adamp(1)/2
     ...
     BCForce = abs(Force)^2;
     PreFactB= BCForce*(1/(abs(waveno)^2)); % constant prefactor for source term
    ...
    Ox=PSx;    %source point coords 
    Oy=PSy;
    Oz=0; 
    
    PreFact= abs(Force)^2/(8*pi*abs(waveno));%for point source
end

PpOm=0.05; %how fine a triangulation to use for result visualisation
    ...
    for jv=1:nVert(j)    %loop over vertices (equiv edges) in the subdomain j where the ray starts
      ...
      %xaj,yaj,zaj = cartsian coords on sending element at ajj
        ...
        % Case 2: pt source at (x,y)=(Ox,Oy,Oz);
               ...
               if FreeEdges(j,jv)==0
                   for loop1=1:Tj(j,jv)
                       for loop2=1:nSideEls(j,jv)                                                      
                            ...
                            ScInt(loop1,loop2)=integral(SR,0,stepb(j,jv));%(abs(Force(loop2+cSideEls(j,jv)))^2)*
            ...
            ScVec(jBlockA:jBlockB,1)=sparse(PreFact.*ScInt(:)./(stepb(j,jv)^tval));
        end
               
     if Adamp(1)<10*MinDamp
