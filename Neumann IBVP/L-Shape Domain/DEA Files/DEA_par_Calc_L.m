function [VecInf,Tj,nSideEls,ThetaLocMat]=DEA_par_Calc_L(dx,waveno,xnodes,ynodes,ForcePre,NQ,t0,Omega,nTotElsL,cElsL)
...
for j=1:nOmega
    ...
    TDNorm(j,:)=cross((XYZ(Omega(j,2),:)-XYZ(Omega(j,1),:)),(XYZ(Omega(j,3),:)-XYZ(Omega(j,1),:))); %normal pointing in z direction                                                                                                   %not used for 2D probs
...
nTotEls=(sum(sum(nSideEls))); %Total No. elements

%Make vector that has entries from edges [1,*,5,6] where * are zeros from
%internal edge in SD1 (sub-domain1)
%cElsL: cummlative no. elements along L shape domain (cEdge from BEM)
%nTotElsL: total no. elements along L shape (M from BEM)
ForceSD1=zeros(cSideEls(1,5),1); %Make vector same size as No. elements around sub-domain 1
ForceSD1(1:cSideEls(1,2),1)=ForcePre(1:cSideEls(1,2),1); %Input elements from the first edge 
ForceSD1(cSideEls(1,3)+1:cSideEls(1,4),1)=ForcePre(cElsL(1,5)+1:cElsL(1,6),1); %Input entries for the top edge of SD1 or 5th edge for L shape
ForceSD1(cSideEls(1,4)+1:cSideEls(1,5),1)=ForcePre(cElsL(1,6)+1:nTotElsL,1);%Input entries for the left edge of SD1
% Similar to above but for Sub-Domain 2 
ForceSD2=zeros(cSideEls(2,5),1);
ForceSD2(1:cSideEls(2,2),1) = ForcePre(cElsL(1,2)+1:cElsL(1,3),1);
ForceSD2(cSideEls(2,2)+1:cSideEls(2,3),1)=ForcePre(cElsL(1,3)+1:cElsL(1,4),1);
ForceSD2(cSideEls(2,3)+1:cSideEls(2,4),1)=ForcePre(cElsL(1,4)+1:cElsL(1,5),1);
%Combine force vectors from each Sub-Domain together
Force=zeros(nTotEls,1);
Force(1:cSideEls(1,5))=ForceSD1;
Force(cSideEls(1,5)+1:nTotEls)=ForceSD2;
...
if Source==0
    ...
    b=min(ynodes);
     ...
     BCForce = abs(Force).^2; % Boundary condition force
