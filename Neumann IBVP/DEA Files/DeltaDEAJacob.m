%DEA code rewritten using delta direction basis line 560
...
format long
%parameters
params.freq = 1000;  %params.omega/(2*pi);                                               % frequency
params.omega = 2*pi*params.freq;  %=real(k);                                  % angular freq
...
%xnodes=[-1 -1 -0.25 -0.25 -0.25 0.25 0.25 0.25 1 1];
%ynodes=[0 0.25 0 0.25 0.5 0 0.25 0.5 0 0.25];
 xnodes=[0 1 1 0];% 
 ynodes=[0 0 1 1];
...
% xnodes=[0 1 (1+s2)/s2 0 1/s2 (1+s2)/s2 0 1 (1+s2)/s2];
% ynodes=[0 0 0  0.5 0.5 0.5  1 1 1];
% znodes=[0  0   -1/s2  0  -1/s2  -1/s2 0  0  -1/s2];

XYZ=[xnodes' ynodes' znodes'];
 
% Define the subdomains (anticlockwise: each row gives a new subdomain)
%
%Omega=[1 3 4 2 0 0; 3 6 7 8 5 4; 6 9 10 7 0 0];
Omega=[1 2 3 4];
%Omega=[1 2 3; 1 3 4];
%Omega=[1 2 5 6; 2 3 4 5];

%Omega=[1 2 4; 4 8 7; 2 5 4; 4 5 8; 2 6 5; 5 6 8; 2 3 6; 6 9 8];
...
Nelt=16;   % Approx no. boundary elts per subsystem 
...
N=4*NQ;
 ...
 sf=2; %scaling factor
 %ThetaAB=linspace(0,2*pi,N+1);%endpoints
 %ThetaC=0.5*(ThetaAB(1:end-1)+ThetaAB(2:end));
% sf=1;
...
n=zeros(nOmega,1);
nSideEls=zeros(nOmega,max(nVert)); %no. of boundary elements on each edge
...
% % %      
    ...
    Adamp(j)=1; %Adamp = imag(k); %imaginary part of k=wavenumber
...
if Source==0
    ...
    a=min(xnodes);
    t0=0*pi;%-0.25*pi;%0;%pi/4;% %source direction
%Helmholtz
     %PreFactB=cos(t0)*params.rhom/2; % constant prefactor for source term
     
     % Helmholtz Neumann BC
     waveno=(params.omega/params.speed)+1i*Adamp(1)/2;
    
     PreFactB= (abs(1/waveno)^2)*cos(t0)*params.rhom/2; % constant prefactor for source term
...
for j=1:nOmega   %loop over subdomains where the ray starts
  ...
  %  if general polygons  
       ...
       YN=ynodes(Omega(j,1:nVert(j)))'; % boundary nodes to calculate solution inside
       pv=[XN YN;XN(1) YN(1)]; 
% %     figure(j)
% %     
      [points,tri]=distmesh2d(@dpoly,@huniform,PpOm,[min(XN),min(YN); max(XN),max(YN)],pv,pv); %genrates internal triangulation 
% % %   
       XP=(points(tri(:,1),1)+points(tri(:,2),1)+points(tri(:,3),1))/3; %interior points where solution calculated
       YP=(points(tri(:,1),2)+points(tri(:,2),2)+points(tri(:,3),2))/3; %at centroid of each trinagle
       ZP=zeros(size(XP));
    
    PlotSize(j)=length(XP);  % tells you how many interior points we evaluate solution at
   
    %quantities for final density (FD) evaulation up to line 340
    ...
    FDScaleV=[];
    %FDValV=zeros(2*NQ*max(nVert),PlotSize(j));
    %FDPosV=zeros(2*NQ*max(nVert),PlotSize(j));
    %LGStoreV=zeros(2*NQ*max(nVert),PlotSize(j));
      ...
      %xaj,yaj,zaj = cartsian coords on sending element at ajj
        ...
        PlotCount=0;%zeros(PlotSize(j),1);
         ...
         for pts=1:PlotSize(j)  
             ...
             if DirSplit(jvp,pts)>DirSplit(jv,pts) %use DirSplit to separate thetac into diections for given edge jv to jvp
                 
                 DirEdge=ThetaC(DirSplit(jv,pts)<ThetaC & ThetaC<DirSplit(jvp,pts));
             else
                 DirEdge=ThetaC(DirSplit(jv,pts)<ThetaC | ThetaC<DirSplit(jvp,pts));
             end             
             ...
             for kk=1:length(Ix)
               ...
               %  pause
                 ...
                 sElt(:,kk)=bsxfun(@gt,sGam(kk)*ones(nSideEls(j,jv),1),ajj); %checks if sGam greater than elt starting with ajj
                                  
                 tList1(1:Tj(j,jv),kk)=bsxfun(@lt,abs(ThetaGam(kk)*ones(Tj(j,jv),1)-ThetaLoc'),10*eps); %check if thetagam in local set of edge angles
...
% MAXIT=1000;
% EPS=1e-8;
% VecInf = bicgstab(LHS,(sf*ScVec),EPS,MAXIT,speye(NM),speye(NM),ones(NM,1));
% 2
clear LHS
...
% 		
 		    Hank=params.rhom*params.omega*dAbsP(j)*params.omega*dAbsP(j)*(abs(besselh(0,1,Zhankre+1i*Zhankim))^2)/16; 
            FD(pts,j)=FD(pts,j)+Hank;       
         end
         %% Exact solution for rectangle
%            if XP(pts)<=0.25
% if YP(pts)>0.25 & YP(pts)<0.75
              ExRect(pts,j)=ExactRectJ(t0, Adamp(j), params.omega, 1./dAbsP(j), sec(t0), params.rhom, XP(pts));
% else
%                  ExRect(pts,j)=0;
% end
         ...
         figure(1)
         trisurf(tri,points(:,1),points(:,2),zeros(size(points(:,1))),log(1+FD(1:PlotSize(j),j)))    
         hold on
         axis equal
         view(2)
          figure(2)
           trisurf(tri,points(:,1),points(:,2),zeros(size(points(:,1))),log(1+ExRect(1:PlotSize(j),j)))
           hold on
           axis equal
           view(2)
...
ErrRectT=sum(abs(ExRect-FD))/sum(abs(ExRect))
